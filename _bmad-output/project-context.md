...
- **`npm run lint` (ESLint 9) is the only automated check — there are no tests.** The target is 0 errors / 0 warnings. ⚠️ **The tree does not currently meet it** — 14 errors / 7 warnings over 12 files, 6 of them in `src/pages/admin/CompetitionSetup.jsx` (the competition module holds 9 of the 14): fix what you touch, never add to the pile.

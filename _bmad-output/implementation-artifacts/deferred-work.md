...
- **Upload du logo sans plafond de taille ni vérification de type réel.** `accept="image/*"` n'est qu'un filtre de sélecteur. Hérité de `AdminVideos.jsx:153` (même code), mais le plan Supabase est gratuit : un fichier de 40 Mo part dans le bucket public sans que rien ne l'arrête. À traiter dans les deux écrans à la fois.
